...
void appMain(gecko_configuration_t *pconfig){
  ...
  conn_params.min_interval 		= 0x15;
  conn_params.max_interval 		= 0x15;
	...
	switch (BGLIB_MSG_ID(evt->header)) {
		...
		case gecko_evt_le_connection_opened_id:
			...
			*/
			...
			gecko_cmd_hardware_set_soft_timer(32768*100,1,0);
		break;
		...
		case gecko_evt_hardware_soft_timer_id:{
			...
			if(evt->data.evt_hardware_soft_timer.handle == 1){
				...
				gecko_cmd_hardware_set_soft_timer(32768*1,2,0);
			}
			if(evt->data.evt_hardware_soft_timer.handle == 2){
